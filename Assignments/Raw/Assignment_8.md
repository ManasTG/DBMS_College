...
---
### Update Table
MariaDB [manas_db]> ALTER TABLE dept ADD location char(50);
Query OK, 0 rows affected (0.014 sec)
Records: 0  Duplicates: 0  Warnings: 0

MariaDB [manas_db]> SELECT * FROM dept;
+--------+------------+----------+
...
MariaDB [manas_db]> UPDATE dept
    -> SET location = 'Mumbai' WHERE deptno = 10;
Query OK, 1 row affected (0.011 sec)
Rows matched: 1  Changed: 1  Warnings: 0

MariaDB [manas_db]> UPDATE dept SET location = 'Chennai' WHERE deptno = 20;
Query OK, 1 row affected (0.070 sec)
Rows matched: 1  Changed: 1  Warnings: 0

MariaDB [manas_db]> UPDATE dept SET location = 'Hyderabad' WHERE deptno = 30;
Query OK, 1 row affected (0.007 sec)
Rows matched: 1  Changed: 1  Warnings: 0

MariaDB [manas_db]> UPDATE dept SET location = 'Delhi' WHERE deptno = 40;
Query OK, 1 row affected (0.007 sec)
Rows matched: 1  Changed: 1  Warnings: 0
...
MariaDB [manas_db]> Insert INTO salgrade VALUES
    -> ('A', 700, 1200),
    -> ('B', 1201, 1400),
    -> ('C', 1401, 2000),
    -> ('D', 2001, 3000),
...
MariaDB [manas_db]> SELECT * FROM salgrade;
+-------+-------+-------+
| grade | losal | hisal |
+-------+-------+-------+
| A     |   700 |  1200 |
...
MariaDB [manas_db]> SELECT e.ename, d.dname
    -> FROM emp e
    -> JOIN dept d
...
MariaDB [manas_db]> SELECT e.ename AS emp, m.ename AS manager
    -> FROM  emp e
    -> JOIN emp m
    -> ON e.mgr = m.empno
    -> WHERE m.ename = 'JONES';
...
MariaDB [manas_db]> SELECT e.ename, e.job, d.dname, m.ename AS manager_name, s.grade
    -> FROM emp e
...
MariaDB [manas_db]> SELECT e.ename, e.job, s.grade, d.dname, e.sal
    -> FROM emp e
    -> JOIN dept d ON e.deptno = d.deptno
    -> JOIN salgrade s ON e.sal BETWEEN s.losal AND s.hisal
    -> WHERE e.job <> 'CLERK'
...
MariaDB [manas_db]> SELECT e.ename, e.job, m.ename AS manager
    -> FROM emp e
    -> LEFT JOIN emp m ON e.mgr = m.empno;
+--------+-----------+---------+
| ename  | job       | manager |
...
MariaDB [manas_db]> SELECT e.ename, e.job, e.sal*12 AS annual_salary,
...
MariaDB [manas_db]> SELECT e.ename, e.empno,
...
MariaDB [manas_db]> SELECT d.dname, d.deptno, SUM(e.sal) AS total_salary
    -> FROM emp e
    -> JOIN dept d ON e.deptno = d.deptno
...
MariaDB [manas_db]> SELECT e.empno, e.ename, d.location
    -> FROM emp e
    -> JOIN dept d ON e.deptno = d.deptno;
+-------+--------+-----------+
